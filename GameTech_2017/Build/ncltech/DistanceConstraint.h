...
public:
	...
	virtual void ApplyImpulse() override
	{
		...
		Vector3 v0 = m_pObj1->GetLinearVelocity()
		 + Vector3::Cross(m_pObj1->GetAngularVelocity(), r1);
		Vector3 v1 = m_pObj2->GetLinearVelocity()
		+ Vector3::Cross(m_pObj2-> GetAngularVelocity(), r2);
		
		{
		float constraintMass =
			 (m_pObj1->GetInverseMass() + m_pObj2->GetInverseMass())
			 + Vector3::Dot(abn,
			Vector3::Cross(m_pObj1->GetInverseInertia()* Vector3::Cross(r1,
				 abn), r1) + Vector3::Cross(m_pObj2->GetInverseInertia()
			 ...
			 float b = 0.0f;
		{
			 ...
			 * distance_offset;
			 ...
			 m_pObj1->SetLinearVelocity(m_pObj1->GetLinearVelocity()
			 + abn *(jn * m_pObj1->GetInverseMass()));
			 m_pObj2->SetLinearVelocity(m_pObj2->GetLinearVelocity()
			 - abn *(jn * m_pObj2->GetInverseMass()));
			 m_pObj1->SetAngularVelocity(m_pObj1->GetAngularVelocity()
				 + m_pObj1->GetInverseInertia()* Vector3::Cross(r1, abn * jn));
			 m_pObj2->SetAngularVelocity(m_pObj2->GetAngularVelocity()
		- m_pObj2->GetInverseInertia()* Vector3::Cross(r2, abn * jn));
		}
