...
void Manifold::SolveContactPoint(ContactPoint& c)
{
	...
	// Collision Resolution
	{
		...
		// Changes here are needed to ensure that :
		// 1. The Collision never solves itself backwards (e.g. pushes itself
		// all the way through the other side of the other object ). As now if
		// an object is colliding on both sides it could theoretically solve
		// it by doing the above scenario .
		// 2. The Friction force never exceeds the force applied by the main
		// collision force . As now the main collision force will change over
		// time .
		
		// ( Inside Collision Resolution block ) Replace :
	...
	// Friction
	{
		...
		if (tangent_len > 0.001f)
		{
			...
			// resolution force
			float oldImpulseTangent = c.sumImpulseFrictionfloat; //( < - - This should
			// be a float not a Vector3 )
			float maxJt = frictionCoef * c.sumImpulseContact;
			c.sumImpulseFrictionfloat =min(max(oldImpulseTangent + jt, maxJt), -maxJt);
			jt = c.sumImpulseFrictionfloat - oldImpulseTangent;
