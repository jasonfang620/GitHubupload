...
bool CollisionDetectionSAT::AreColliding(CollisionData* out_coldata)
{
	...
	// either return false , or return the best axis (one with the least
	// penetration ) found .
...
void CollisionDetectionSAT::FindAllPossibleCollisionAxes()
{
	...
	// direction is mid way between two default collision axes provided
	// above .
	...
	// If both are spheres then the only axes we have to check is
	// between the two centre points
	...
	// If only shape1 is a sphere then we have to get the closest point
	// on the edge of shape2 to use as an axis

	else if (shape1_isSphere)
	{
		Vector3 p =
			GetClosestPoint(m_pObj1->GetPosition(), shape2_edges);
	...
	// If only shape2 is a sphere then we have to get the closest point
	// on the edge of shape1 to use as an axis

	else if (shape2_isSphere)
	{
		Vector3 p =
			GetClosestPoint(m_pObj2->GetPosition(), shape1_edges);
...
void CollisionDetectionSAT::GenContactPoints(Manifold* out_manifold)
{
	...
	{
	out_manifold->AddContact(polygon2.front()- m_BestColData._normal * m_BestColData._penetration,polygon2.front(), m_BestColData._normal,
	...
	{
	// Set up with shape2 becoming incident face and shape1
	// being the reference
		...
		// Compute furthest point along the reference face along the
		// collision normal - For computing new penetration_depth
		// estimate for each contact point
		
		penetration_offset = -FLT_MAX;
		for (Vector3 & v : polygon1)
		{
		float poffset = Vector3::Dot(v, m_BestColData._normal);
		
		if (poffset > penetration_offset)
		{
		 penetration_offset = poffset;
		 }
		};
		...
		{
		// Set up with shape1 becoming incident face and shape2
		// being the reference
		...
		if (incPolygon->size() > 0)
		{
			...
			for (const Vector3 & endPoint : *incPolygon)
				...
				contact_penetration = -(Vector3::Dot(endPoint,m_BestColData._normal) - penetration_offset);
